...
export class AuthHelper {
    public static isValid(telegramInitData: string, botToken: string, hash: string): boolean {
    ...
    private static parseInitData(telegramInitData: string): Pair<string, string> {
    ...
    private static parseQueryString(queryString: string): Map<string, string> {
        ...
        for (const pair of pairs) {
            ...
            const key = idx > 0 ? decodeURIComponent(pair.substring(0, idx)) : pair;
            const value = idx > 0 && pair.length > idx + 1 ? decodeURIComponent(pair.substring(idx + 1)) : null;
            if (value !== null) {
    ...
    private static createHmac(algorithm: string, key: string, data: string): Buffer {
